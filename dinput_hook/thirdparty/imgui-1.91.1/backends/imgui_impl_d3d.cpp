...
#else
#define IMGUI_COL_TO_D3D_ARGB(_COL)                                                                \
...
static ImGui_ImplD3D_Data *ImGui_ImplD3D_GetBackendData() {
    return ImGui::GetCurrentContext()
               ? (ImGui_ImplD3D_Data *) ImGui::GetIO().BackendRendererUserData
               : NULL;
}
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != NULL) {
                ...
                hr_assert(bd->pd3dDevice->DrawIndexedPrimitive(
                    D3DPT_TRIANGLELIST, D3DFVF_CUSTOMVERTEX,
                    vertices.begin() + pcmd->VtxOffset + global_vtx_offset,
                    (UINT) cmd_list->VtxBuffer.Size, cmd_list->IdxBuffer.Data + pcmd->IdxOffset,
                    pcmd->ElemCount, 0));
            }
        }
...
static bool ImGui_ImplD3D_CreateFontsTexture() {
    ...
    for (int y = 0; y < height; y++)
        memcpy((unsigned char *) surface_desc.lpSurface + (size_t) surface_desc.lPitch * y,
