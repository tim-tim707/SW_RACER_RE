...
// left-anchored (centering is phase 2).
//
...
// must be made writable once (VirtualProtect) before patching.
void swrSprite_GetUIScale_delta(float *out_xscale, float *out_yscale) {
    // --- sprites ---
    double recipH = *(double *) 0x004acce0;
    float yscale = (float) ((double) swrDisplay_screenHeight * recipH);
    *out_xscale = yscale;
    *out_yscale = yscale;

    // --- text (patch Add2DQuad2's X reciprocal to match its Y reciprocal) ---
    ...
    if (text_recip_writable && swrDisplay_screenWidth != 0) {
        double textRecipH = *(double *) 0x004ac630;
        *(double *) 0x004ac628 =
