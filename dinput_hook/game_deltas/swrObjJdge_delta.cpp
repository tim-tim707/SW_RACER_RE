...
// Native 2-player bug: KeyDownForPlayer1Or2 (0x0045e120) falls through and returns the (nonzero)
// mask instead of 0 when numLocalPlayers >= 2 and NEITHER player is pressing (disasm at 0x0045e19c),
// so pause / HUD-cycle / in-race menu fire every frame -- the "stuck paused, unpause re-pauses"
// symptom. Only the no-press case is corrected. Inert in single-player.
...
// Splitscreen speed-dial fix. Each local player owns its own fill gradient sprite (P1 = 0xf speed /
// 0x10 overheat, P2 = 0x13 / 0x14 -- the +4 offset in the shared dial drawer FUN_0045fe70), but the
// fill pipeline is single-player, keyed on two globals (swrSpriteTexture_dial_gradient_rgb / _rgb2)
// that swrPlayerHUD_LoadRaceHUD only ever points at P1's sprites:
//   * swrSprite_Draw (0x0044f160) chooses a trimmed fill polygon (rdProcEntry_Add2DPolygon) vs a
//     full quad (rdProcEntry_Add2DQuad3) by matching the sprite's texture against those globals, so
//     P2's fill sprites take the full-quad branch and P2's bar draws permanently full.
//   * The ratios are two scalars, speedDialPosition1 / 2, written unconditionally for whichever
//     player swrRace_InRaceTimer processed last -- so P1's bar shows P2's speed.
// Fixed by snapshotting each player's ratio + fill-sprite textures in swrRace_InRaceTimer_delta,
// then re-pointing the gradient global and ratio per sprite in swrSprite_Draw_delta so both gates
// inside the original agree. Gated on numLocalPlayers >= 2.
...
void swrRace_InRaceTimer_delta(void *score, void *jdge) {
    ...
    // speedDialPosition2 is only refreshed while overheating, but the overheat fill sprite is
    // hidden otherwise, so a stale value is never drawn.
    s_speedDialFill[idx] = speedDialPosition1;
    s_overheatDialFill[idx] = speedDialPosition2;
    // Fill sprites are (re)assigned at HUD load, so the texture table is refreshed each pass.
...
// Splitscreen opponent-marker fix. swrPlayerHUD_RenderDistanceText gates each marker on
// projectedDepth < player_sprite_depth_values[i], but swrPlayerHUD_SampleOcclusion only samples the
// PRIMARY viewport's projected positions (player_sprite_pixel_pos_x/y), never the secondary set
// (_x2/_y2) -- so on P2's pass the depth array holds P1's view (or the -1000 default) and every
// marker fails. The occlusion test is already inert on 32-bit displays (the sampler only handles
// 8/16-bit, see swrPlayerHUD.h), so the secondary pass raises the depths out of the way.
...
// Splitscreen P2 boost. swrRace_BoostCharge (0x0046bd20) keeps its charge state on the pod
// (player+0x210, already per-pod) but reads only main-device globals, all written from P1's device:
//   * CHARGE reads swrRace_ThrustInput / swrRace_ThrottleInput / swrRace_PitchInput.
//   * FIRE (state +0x210 == 2) reads swrRace_BoostInput.
// So P2's own input is swapped into those four around the original call and restored after, leaving
// P1's boost / force-feedback path untouched. The timed START boost keys off the main-device
// hold-time accumulators DAT_00ec88a0[3]/[4], so P2's own accumulated hold-times are substituted
// too -- feeding 0 would deny P2 the start boost, feeding P1's would let P1's throttle trigger it.
// These globals are read ONLY by the boost paths in the indexed control path, so the swap is inert
// outside boost. Gated on numLocalPlayers >= 2.
...
// Main-device hold-time accumulators DAT_00ec88a0[3]/[4] (thrust / boost), read by the flags1
// afterburner / timed-start-boost block.
static float *const kHoldTimeThrust = (float *) 0x00ec88ac;
static float *const kHoldTimeBoost = (float *) 0x00ec88b0;
// Accumulated the way swrControl_ProcessInputs does for the main device: += dt while held, reset on
// release. The timed start boost needs these.
...
unsigned int swrObjJdge_InitTrack_delta(swrObjJdge *judge, swrScore *scores) {
    ...
    // Stamp the 2nd roster slot as 'Locl' before the original counts identifiers: it derives
    // numLocalPlayers purely from that count, and a count > 1 sets the splitscreen master flag
    // (DAT_0050ccf0) and assigns secondLocalPlayer, cascading into the split viewport/camera/HUD.
    ...
    // InitTrack assigns secondLocalPlayer, but the dropped 2-player front-end never gives it a
    // controller: swrObjHang_BuildRosterMultiplayer only sets the single local player's control
    // config (score+0xc, a stride-0x50 entry whose +0x23 byte is the control type) and control index
    // (score+0x10 low byte). Without them P2 reads control index 0 / a null config.
    if (swrObjJdge_forceSplitscreen && numLocalPlayers >= 2 && firstLocalPlayer && secondLocalPlayer) {
        ...
        if (p1cfg) {
            ...
            *(uintptr_t *) ((char *) secondLocalPlayer + 0xc) = p2cfg;
            // Indexed scheme 2 (avoids a case-1 quirk), so UpdatePlayerControl routes P2 through
            // the per-player array/bitset path rather than the type-0 "main device" path that reads
            // the shared global -- which made P1's stick drive both pods.
