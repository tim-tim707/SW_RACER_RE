...
//----- (00421120) --------------------------------------------------------
signed int __cdecl parse_racer_tab(const char *a1)
{
    int v4; // ebp
    char *v10; // edi
    char *v12; // esi
    ...
    qsort(dword_4EB3C4, dword_4EB3CC, 4u, string_compar);
...
const char *__cdecl lookup_translation(const char *a1)
{
    ...
    if (!a1)
        return 0;
    if (!*a1 || *a1 != '/' || strlen(a1) == 1)
        return a1;
    v2 = strchr(a1 + 1, '/') + 1;
    if (!lpSrcStr)
        return v2;
    strncpy(&SrcStr, a1 + 1, 254);
    v3 = strchr(&SrcStr, '/');
    if (v3)
        *v3 = 0;
    sub_4AB5D0(&SrcStr);
    v4 = (const char **)bsearch(&v5, dword_4EB3C4, dword_4EB3CC, 4u,
                                string_compar);
    if (!v4)
        return v2;
