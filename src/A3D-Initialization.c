...
signed int __cdecl parse_audio_config(int a1)
{
    ...
    while (read_line_unk())
    {
        ...
        if (!_strcmpi(config_string_EC8E84, aAudio))
        {
            if (!_strcmpi(dword_EC8E8C, aHires))
            {
                use_some_wave_file_4B6D14 = _strcmpi(dword_EC8E90, aOn) == 0;
            }
            ...
            {
                a3d_control_something_4B6D18 = _strcmpi(dword_EC8E90, aOn) == 0;
            }
...
void __cdecl sub_484D40(float a1)
{
    if (a1 < 0.0f)
    {
        volume_setting_4C7D7C = 0.0f;
    }
    else if (a1 > 10.0f)
    {
        volume_setting_4C7D7C = 10.0f;
    }
    else
    {
        volume_setting_4C7D7C = a1;
    }
    return;
}
...
signed int sub_4848A0()
{
    ...
    HRESULT v1 = sub_49E970(0, &IA3dX_50D548, 0, 128);
    if (v1 >= 0 && IA3dX_50D548)
    {
        dword_50D550 = 1;
    }
    else
    ...
    int v2 = get_hwnd();
    if (IA3dX_50D548->SetCooperativeLevel(v2, 1) < 0)
    {
    ...
    set_output_gain(dword_4C7D70);

    if (dword_50D550)
    {
        if (IA3dX_50D548->QueryInterface(&unk_4AE158, &dword_50D560) < 0)
        {
            dword_4C7D80 = 0;
            dword_50D550 = 0;
            dword_50D560 = 0;
...
//----- (00484A80) --------------------------------------------------------
void __cdecl set_output_gain(float a1)
{
    if (IA3dX_50D548)
    {
...
//----- (00484D90) --------------------------------------------------------
int __cdecl set_gain(A3dSource *a1, float a2)
{
    if (!IA3dX_50D548)
    {
        return IA3dX_50D548;
    ...
    if (get_a3dsource_rendermode(a1) & A3DSOURCE_RENDERMODE_NATIVE)
    {
        a2 *= volume_setting_4C7D7C;
    }
...
//----- (00485040) --------------------------------------------------------
int __cdecl get_a3dsource_rendermode(A3dSource *a1)
{
    DWORD v4 = -1; // [esp+8h] [ebp-4h]
    a1->GetRenderMode(&v4); // A3DSOURCE_RENDERMODE, but which one ?
    return v4;
}

//----- (00484E10) --------------------------------------------------------
int __cdecl set_listener_position(A3dSource *a1, const float *a2)
{
