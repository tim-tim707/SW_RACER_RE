...
##tgfd
    .dat file
        ...
        What a mess..this mirrors the current profile file,
    so they can desync too.Also since this keeps track of the freeplay stuff,
    that can also desync with the current profile.
...
```C
    ...
    typedef struct
{
    uint8_t race_unlocked[4]; // 0xC 1 bit per course starting at LSb: unlocks
                              // race for freeplay
    uint8_t podracer_unlocked[4]; // 0x10 1 bit per podracer ??? for freeplay
    Savedata profiles[4]; // 0x14 4 profiles, but the first profile seems to be
                          // the one used for most things
    ...
    //
    float time_seconds[100]; // 0x154 default value seems to be 3599.9899902344
                             // (0x4560FFD7) which is about an hour
    char time_names[100][32]; // 0x2E4 default value seems to be 0x41 ('A')
    uint8_t time_podracer[100]; // 0xF64 default value seems to be the track
                                // favorite
...
```
    ...
    ##sav files
        ...
        Note that this info is typically loaded from tgfd.dat So if you modify
            something,
    you should delete that file first

```C
    // This data follows the 4 byte magic in the savegames (from
    // data/player/*.sav) They come from 0xE364B4 in memory
    typedef struct
{
    char profileName[32]; // 0x00, size correct?
    uint8_t unk[4]; // 0x20
    uint8_t last_podracer; // 0x24 value:
                           //   0x01 = Anakin Skywalker ??? Probably 0x00.
                           //   Teemto somethingsomething seems to be 0x01 0x09
                           //   = Ebe Endocott 0x0A = Dud Bolt 0x0B = Gasgano

    uint8_t
        race_unlocked[5]; // 0x25 1 bit per course starting at LSb: unlocks race
    uint16_t race_status[5]; // 0x2A 2 bit per course starting at LSb: 0=4th (or
                             // not finished), 1=3rd, 2=2nd, 3=1st.
    ...
    BOOL __cdecl sub_4219D0(int a1)
{
    ...
    if (!fwrite_locked(&v5, 1u, 4u, v3))
    {
        v1 = 1;
    }

    // Write actual data
    if (!fwrite_locked(v6, 1u, 0x50u, v3))
    {
        v1 = 1;
    }
